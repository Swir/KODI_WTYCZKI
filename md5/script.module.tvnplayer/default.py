    ...
    if m == 'mainInfo':
        ...
        for item in categories:
            ...
            if type != 'titles_of_day' and type != 'favorites' and type != 'pauses':
                addDirTVN(name,'getItems',type,id,'DefaultVideoPlaylists.png','','')
    else:
        ...
        if type == "series":
            if njson['items'][0]['season'] == str(season):
                ...
                for item in seasons:
                    ...
                    addDirTVN(name,'getItems',type,id,'DefaultTVShows.png','',season)
                if not seasons:
                    return TVNPlayerItems(njson)
        else:
            return TVNPlayerItems(njson)
        ...
        for item in items:
            ...
            if type == 'episode':
                if clickable == 1 :                
                    ...
                    addLinkTVN(name,url,thumbnail,gets,tvshowtitle,lead,episode,season,start_date)
            else:
                addDirTVN(name,'getItems',type,id,thumbnail,gets,'')
...
def htmlToText(html):
#    xbmcplugin.setContent(int(sys.argv[1]), 'movies') # "Zawartość - Filmy"
...
def addDirTVN(name,m,type,id,thumbnail,gets,season):
#        xbmcplugin.setContent(int(sys.argv[1]), 'movies') # "Zawartość - Filmy"
#        xbmc.executebuiltin('Container.SetViewMode('+WIDOK_SET+')') # Zmiana widoku wedłóg ustawień
...
def addLinkTVN(name,url,thumbnail,gets,serie_title,lead,episode,season,start_date):
#        xbmcplugin.setContent(int(sys.argv[1]), 'movies') # "Zawartość - Filmy"
#        xbmc.executebuiltin('Container.SetViewMode('+WIDOK_SET+')') # Zmiana widoku wedłóg ustawień
        ok=True
        if not gets:
